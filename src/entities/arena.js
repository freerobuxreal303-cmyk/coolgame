...
// Implements Celestial Sanctuary map layout, Cores, Turrets, Grav-Wells, and Bounce Bumpers.
...
class CoreBase {
  ...
  render(ctx) {
    this.pulse += 0.05;
    const glow = Math.sin(this.pulse) * 4;
    ...
    ctx.fillRect(-barW / 2, -this.radius - 22, barW, barH);
    ctx.fillStyle = this.team === 'player' ? '#00ff66' : '#ff3366';
    ctx.fillRect(-barW / 2, -this.radius - 22, barW * ratio, barH);
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 1.5;
    ctx.strokeRect(-barW / 2, -this.radius - 22, barW, barH);

    // Text Label
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 12px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(this.team === 'player' ? 'PLAYER NEXUS CORE' : 'ENEMY MATRIX CORE', 0, -this.radius - 28);
    ctx.fillText(`${Math.ceil(this.hp)} / ${this.maxHp}`, 0, -this.radius - 8);
...
class DefenseTurret {
  constructor(x, y, team = 'player') {
    ...
    this.range = 550;
    this.attackCd = 0;
    this.attackRate = 1.1; // Seconds per shot
    this.isDead = false;
    this.target = null;
    this.aimAngle = 0;
  }
  ...
  fire(projectiles, soundEngine) {
    ...
    projectiles.push(new Projectile({
      x: this.x + Math.cos(this.aimAngle) * 35,
      y: this.y + Math.sin(this.aimAngle) * 35,
      vx,
      vy,
      radius: 8,
      damage: 260,
  ...
  render(ctx) {
    ...
    if (this.target) {
      ctx.strokeStyle = this.team === 'player' ? 'rgba(0, 229, 255, 0.25)' : 'rgba(255, 23, 68, 0.25)';
      ctx.lineWidth = 1;
      ctx.setLineDash([6, 6]);
      ctx.beginPath();
      ctx.moveTo(0, 0);
...
class Arena {
  constructor(width = 2400, height = 1100) {
    ...
    this.turrets = [
      new DefenseTurret(550, 320, 'player'),
      new DefenseTurret(550, 780, 'player'),
      new DefenseTurret(width - 550, 320, 'enemy'),
      new DefenseTurret(width - 550, 780, 'enemy'),
    ];
  ...
  render(ctx) {
    ...
    for (const gw of this.gravityWells) {
      ...
      ctx.beginPath();
      ctx.arc(gw.x, gw.y, 24, 0, Math.PI * 2);
      ctx.fill();
